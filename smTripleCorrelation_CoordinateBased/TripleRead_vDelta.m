function [triple_trans, results, coordinates, coordinates_norm] = TripleRead_vDelta(triplemat, PC_1, PC_2, PC_3, boxradii, dim, rho_res, NormMode, normDist, TripleCalSeq, TripleDispSeq)
% ========================= Sequence Info =================================
seq = zeros(1,3);
...
for ii = 1 : SizeofTriple(1)
    ...
    for jj = 1 : SizeofTriple(2)
        ...
        for kk = 1 : SizeofTriple(3)
            ...
            triplemat(ii, jj, kk) = (triplemat(ii, jj, kk)-1) - (PC_1(ii, 2)-1) - (PC_2(jj, 2)-1) - (PC_3(trans_kk, 2)-1);
        end
    end
end
       
% ==================== building up transforming cube ======================
triple_trans = zeros(dim, dim, dim);
...
PC_1(:, 2) = smoothdata(PC_1(:, 2), 'gaussian', 2*boxradii(1)+1);
PC_2(:, 2) = smoothdata(PC_2(:, 2), 'gaussian', 2*boxradii(2)+1);
PC_3(:, 2) = smoothdata(PC_3(:, 2), 'gaussian', 2*boxradii(3)+1);

evaluate = triple_trans(:);
evaluate(evaluate < 0) = [];

threshold = mean(evaluate) + 2.5 * std(evaluate);
disp(['mean = ' num2str(mean(triple_trans(:))) ', std = ' num2str(std(triple_trans(:))) ', max = ' num2str(max(triple_trans(:))) ', thresh = ' num2str(threshold)]);
...
delta123 = triple_trans(indx);
delta23 = PC_3(indx_23, 2) - 1;
delta12 = PC_2(indx_12, 2) - 1;
delta13 = PC_1(indx_13, 2) - 1;
...
for k = 1 : size(indx, 1)
    ...
    if triple_trans(indx_13(k), indx_12(k), indx_23(k)) < max(subvolume(:))
        mask(k) = 1;
        continue
    end
...
if isempty(indx_23)
    ...
    for ii = 1 : dim
        for jj = 1 : dim
            kklb = max(abs(ii - jj), 1);
            kkub = min(ii + jj, dim);
            [max_tmp, kk] = max(squeeze(triple_trans(ii, jj, kklb:kkub)));
...
results = [indx_13.*rho_res, indx_12.*rho_res, indx_23.*rho_res, delta123, delta13, delta12, delta23];
