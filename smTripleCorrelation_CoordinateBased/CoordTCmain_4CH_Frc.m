...
addpath('C:\Users\RothenbergLab\Documents\MATLAB\MATLAB\smTripleCorrelation\');

% Read Data

path0 = 'Z:\rothenberglab\homes\yiny02\Data_2018\GQ_Results\Fraction_Exp_Test\Output_MFA_20180806ATRCHK1';
...
for i = 1 : numel(SampleNames)
    ...
    %}   
    if ~exist([path0 '\' SampleNames{i} '\TripleFrcs_RBG'],'dir')
        mkdir([path0 '\' SampleNames{i} '\TripleFrcs_RBG']);
    end
    ...
    for j = 1 : numel(spools)
        ...
        for r = 1 : size(vnRectBounds,1)
            ...
            if ~isempty(x_R) && ~isempty(x_G) && ~isempty(x_B)
                tic;
                [TripleResult_RBG, ~, rad_center] = smTripleCorrelation_Frc_CPU(coor_R, coor_B, coor_G, roiszx, roiszy, MaxRhoSize, rho_res, rad_res);
                if ~isempty(TripleResult_RBG)
                    save([path0 '\' SampleNames{i} '\TripleFrcs_RBG\' fname '_triple_' num2str(r) '.mat'],'TripleResult_RBG');
                end
                toc;
            end
