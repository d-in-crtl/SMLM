function [triple_trans, results, coordinates, coordinates_norm] = TripleRead_Frc(triplemat, boxradii, dim, rho_res, NormMode, normDist, TripleCalSeq, TripleDispSeq)
% ========================= Sequence Info =================================
seq = zeros(1,3);
...
triple_trans = imgaussfilt3(triple_trans, boxradii./2);

evaluate = triple_trans(:);
threshold = mean(evaluate) + 3 * std(evaluate);
...
cos_theta_1 = ((indx_12-0.5).^2+(indx_13-0.5).^2-(indx_23-0.5).^2)./(2.*(indx_12-0.5).*(indx_13-0.5));
cos_theta_2 = ((indx_12-0.5).^2+(indx_23-0.5).^2-(indx_13-0.5).^2)./(2.*(indx_12-0.5).*(indx_23-0.5));
cos_theta_3 = ((indx_13-0.5).^2+(indx_23-0.5).^2-(indx_12-0.5).^2)./(2.*(indx_13-0.5).*(indx_23-0.5));
...
results = [(indx_13-0.5).*rho_res, (indx_12-0.5).*rho_res, (indx_23-0.5).*rho_res, I];

% ======================== calculate the coordinates ======================

[coordinates, coordinates_norm] = Edge2Coor((indx_13-0.5).*rho_res, (indx_12-0.5).*rho_res, (indx_23-0.5).*rho_res, seqmode, NormMode, normDist, 'N');
