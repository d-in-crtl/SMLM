...
map_ini = [103,0,31; 178,24,43; 214,96,77; 244,165,130; 253,219,199; 247,247,247; 209,229,240; 146,197,222; 67,147,195; 33,102,172; 5,48,97] ./ 255;
...
if (lims(1) < 0) && (lims(2) > 0)
    ...
    if ratio >= 0.5
        seg = round(5.5 / ratio);
        start = max(11 - seg, 0) + 1;
        mid = round((start + 6) / 2);
        top = map_ini(start, :);
        topmiddle = map_ini(mid, :);
    else
        seg = min(round(5.5 / (1 - ratio)), 11);
        mid = round((seg + 6) / 2);
        bottom = map_ini(seg, :);
        topmiddle = map_ini(mid, :);
    end
