...
path0 = 'Z:\archive\Chelsea\000 2019-10 post G4 paper\000_SW1271\03012021_EdUPARP2MCMPCNA_1hrtala\Output_Gauss';
...
for i = 1 : numel(SampleNames)
    ...
    if ~exist([path0 '\' SampleNames{i} '\TripleResults_' CalMode],'dir')
        mkdir([path0 '\' SampleNames{i} '\TripleResults_' CalMode]);
    end
    ...
    for j = 1 : numel(spools)
        ...
        for r = 1 : size(vnRectBounds,1)
            ...
            %coor_D(isnan(coor_D(:, 1)) | isnan(coor_D(:, 2)), :) = [];
            coor_R = [x_R, y_R] .* original_pxsz;
            coor_R(isnan(coor_R(:, 1)) | isnan(coor_R(:, 2)), :) = [];
            coor_G = [x_G, y_G] .* original_pxsz;
            coor_G(isnan(coor_G(:, 1)) | isnan(coor_G(:, 2)), :) = [];
            coor_B = [x_B, y_B] .* original_pxsz;
            coor_B(isnan(coor_B(:, 1)) | isnan(coor_B(:, 2)), :) = [];
