...
clc
fclose('all');
addpath('C:\Users\RothenbergLab\Documents\Visual Studio 2015\Projects\MATLAB\sCMOS\smPairCorr\CoordinatesBased');
addpath('C:\Users\RothenbergLab\Documents\Visual Studio 2015\Projects\MATLAB\sCMOS\ROIRender');

ColorMode = 'BRG'; % the autocorrelation will be calculated for all the colors specified here;
...
for s = 1 : numel(SampleNames)
    ...
    for sp = 1 : numel(SpoolZips)
        ...
        for r = 1 : size(vnRectBounds,1)
            ...
            tic; 
            [roi, PrecisionCnts] = ROIRender_v1129(reports, vnRectBounds(r,:), precision_bin, Camera_pxsz, Target_pxsz, ReconMode);
    ...
    options = optimoptions(@lsqcurvefit,'Algorithm','trust-region-reflective','MaxFunEvals',40000,'MaxIter',2000,'TolFun',1e-8,'TolX',1e-8);
