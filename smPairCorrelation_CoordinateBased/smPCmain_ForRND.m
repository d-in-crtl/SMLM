...
clc
fclose('all');
addpath('C:\Users\ERothenberg\Documents\MATLAB\smPairCorr\CoordinatesBased');
addpath('C:\Users\ERothenberg\Documents\MATLAB\ROIRender');
...
for s = 1 : numel(SampleNames)
    ...
    for sp = 1 : numel(SpoolZips)
        ...
        for r = 1 : size(vnRectBounds,1)
            ...
            tic; 
            [roi, ~] = ROIRender_v1129(reports, vnRectBounds(r,:), precision_bin, Camera_pxsz, Target_pxsz, ReconMode);
            
            %---------------------- generates a random r ------------------
            r_rnd = datasample((1:size(vnRectBounds_rnd,1)), 1);
            %--------------------------------------------------------------
            [roi_rnd, ~] = ROIRender_v1129(reports_rnd, vnRectBounds_rnd(r_rnd,:), precision_bin, Camera_pxsz, Target_pxsz, ReconMode);
