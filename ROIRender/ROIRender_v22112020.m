function [roi, PrecisionCnts] = ROIRender_v22112020(reports, vnRectBounds, precision_bin, original_pxsz, target_pxsz)
...
for ii = 1 : size(reports, 2)
    ...
    Source(ii).precision = (reports(ii).table(:, end-1) + reports(ii).table(:, end-2)); % Unit: original_pxsz
